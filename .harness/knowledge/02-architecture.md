...
- 表现层（`fe/css/` + `fe/js/components/`）：ITCSS+BEM CSS 架构，ES6 Module 组件。主要视图：Overview（系统状态）、Jobs（任务卡片+执行历史）、Sessions（会话列表+详情）、Tools（Skills/MCP/Plugins）
- 服务层（`fe/js/services/`）：API 调用封装（fetchJson/postJson）。统一错误处理，返回 Promise
- 工具层（`fe/js/utils/`）：常量定义（API_BASE、REFRESH_INTERVAL）、DOM 操作工具（escapeHtml、linkify、formatNumber）
- 后端 API 层（`be/be/server.py`）：Starlette HTTP 服务器、Route 路由分发、StaticFiles 静态资源、数据解析（Markdown -> JSON、SQLite -> JSON）
...
- 后端 be/server.py 仅负责文件解析与 HTTP 响应，不修改 Hermes 数据
...
- be/server.py 仅监听 localhost:8090，不对外暴露
