...
- 入口：`fe/html/index.html`（前端 SPA）+ `be/server.py`（后端 API 服务器）
- 启动：`be/scripts/start-http-exec.sh` 由 launchd 调起，检查端口可用性后启动 be/server.py
- 根状态：前端 JavaScript 维护 jobs 列表数据，通过 API 轮询刷新（每 5 分钟自动刷新）

## 核心流程

1. 服务启动：launchd → be/scripts/start-http-exec.sh → be/server.py 监听 8090 端口
2. 数据加载：be/server.py 读取 ~/.hermes/cron/jobs.json 获取任务配置，扫描 output/ 目录获取执行记录
